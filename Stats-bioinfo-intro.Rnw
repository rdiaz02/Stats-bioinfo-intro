...
% \begin{frame}
% \frametitle{What this class is not}
% \begin{itemize}
% \item Intro to stats.
% \item All the stats you need to analyze your data.
% \item A stats recipes book.
% \item Note: we will use examples that are much, much, much simpler than
%   almost anything you'll ever deal with.
% \end{itemize}
% \end{frame}
...
% \begin{frame}
%   \frametitle{DNA, genes and probes (spots)}
%   \includegraphics[width=10.0cm,
%      keepaspectratio]{DNA.pdf}
%  \end{frame}



%% \begin{frame}
%% \frametitle{High-throughput et al.}
%% \begin{itemize}
%% \item The following provides some intro slides about microarrays.
%% \item Microarrays can be used to measure messenger RNA (mRNA) or gene
%%   expression. Similar approaches for array CGH (messure copy number in
%%   genomic DNA). Similar approaches for \ldots
%% \item The technical details are not important. What matters: tens or
%%   hundreds of subjects for which we measure tens or hundreds of thousands
%%   of genes or probes.
  
%% \item What really matters: a few hundreds of subjects, and for each one of
%%   them information on tens of thousands to millions of characters (genes,
%%   probes, SNPs, whatever).

%% \end{itemize}
%% \end{frame}

%% \begin{frame}[fragile]
%% \frametitle{IntroducciÃ³n a las microarrays}

%% Material from Gema Moreno Bueno (Dept. BioquÃ­mica, Universidad AutÃ³noma de
%% Madrid).

%% \vspace*{20pt}
%% (The slides are about measuring gene expression, mRNA. It is assumed that
%% we can do \verb= mRNA -> cDNA =).
...
%% \begin{itemize}
%% \item We want to make inferences in a noisy world.
%% \item Stats tells us how to go from data to conclusions (or lack of
...
\begin{frame}
\begin{textblock*}{105mm}(10mm,11mm)
\textblockcolour{}
\includegraphics[%
    width=10.5cm,
